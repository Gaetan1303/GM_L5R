...
// Filtered/paginated schools (Lot 2 canonical)
...
router.get('/avantages/:id', async (req, res) => {
    try {
        ...
        const advantage = data.advantages.find(a => a.id === req.params.id);
        if (!advantage) {
            return res.status(404).json({ error: 'Avantage introuvable' });
        }
        res.json(advantage);
    } catch (error) {
...
router.get('/desavantages/:id', async (req, res) => {
    try {
        ...
        const disadvantage = data.disadvantages.find(d => d.id === req.params.id);
        if (!disadvantage) {
            return res.status(404).json({ error: 'Désavantage introuvable' });
        }
        res.json(disadvantage);
    } catch (error) {
...
// Lot 3: Social, Economy, Travel, and Formulas endpoints
...
router.get('/scenario-guide', async (req, res) => {
    try {
        ...
        res.status(404).json({ error: 'Scenario guide not available yet' });
    }
});
...
router.get('/scenario-guide/structured', async (req, res) => {
    try {
        ...
        const pickTop = (arr, n = 5) => arr.slice(0, n);
        ...
        const summarize = (s) => ({
            source: s.source,
            titre: s.titre,
            excerpt: (s.paragraphs || []).slice(0, 3)
        });
        ...
        res.status(404).json({ error: 'Structured scenario guide not available yet' });
    }
});
